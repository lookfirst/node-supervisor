...
function run (args) {
  ...
  // store the call to startProgram in startChildProcess
  // in order to call it later
  startChildProcess = function() { startProgram(executor, program); };
...
/* exec is node. prog is the javascript file, plus any arguments. */
function startProgram (exec, prog) {
  var args = prog;
  if (nodeArgs) {
    args = nodeArgs.concat(prog);
  }
  crash_queued = false;
  util.debug("Starting child process with '" + exec + " " + args.join(" ") + "'");
  var child = exports.child = spawn(exec, args);
